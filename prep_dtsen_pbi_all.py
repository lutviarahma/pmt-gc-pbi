...
def stage_1_cleaning():
    ...
    print(f"[INFO] Raw counts — Root: {len(df_root_raw)}, AK: {len(df_ak)}, Meteran: {len(df_meteran)}")
    ...
    print(f"[INFO] Root setelah filter lantai: {len(df_root)} ({len(df_root_raw)-len(df_root)} di-drop)")
    ...
    if 'ak_keberadaan_value' in df_ak.columns:
        ...
        print(f"[INFO] AK setelah filter keberadaan: {len(df_ak)} ({n_before-len(df_ak)} di-drop)")
    ...
    if lost_ids:
        pd.Series(list(lost_ids)).to_csv("lost_ids_stage1.csv", index=False, header=['assignment_id'])
        print(f"[REKON] Lost IDs disimpan ke lost_ids_stage1.csv")
...
def stage_2_imputation(merged_file):
    ...
    if 'ak_sekolah_value' in df.columns:
        ...
        n_imputed = mask_balita.sum()
        df.loc[mask_balita, 'ak_sekolah_value'] = 0
        print(f"[INFO] Imputasi balita (umur 0-4): {n_imputed} baris")

        # Drop invalid age/school (9 = kode tidak tahu / error)
        df = df[
            df['ak_umur'].notna() &
    ...
    print(f"[INFO] Baris di-drop saat imputasi: {n_dropped} | Sisa: {len(df)}")
...
def stage_3_feature_engineering():
    ...
    m = {
        ...
        'luas': 'luas_lantai', 'smart': 'pengeluaran_pulsa', 'internet': 'pengeluaran_internet'
    }
    ...
    asset_map = {
        'ac': 'ac', 'lemaries_kulkas': 'fridge', 'gas_5kg': 'lpg5kg',
        'komputer_laptop': 'computer', 'emas_perhiasan': 'jewelry',
        'sepeda_motor': 'motorcycle', 'mobil': 'car', 'jumlah_lahan_lain': 'land'
    }
    asset_cols = []
